...
class Test_Tie_Point_Grid(unittest.TestCase):
    ...
    def test_to_PointShapefile(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(os.path.isfile(outpath))
            self.assertEqual(_get_n_records(f'{outpath[:-4]}.dbf'), n_all_points - n_nodata)

        with tempfile.TemporaryDirectory() as tmpdir:
            outpath = os.path.join(tmpdir, 'test_out_shapefile_incl_nodata.shp')
            self.TPG.to_PointShapefile(outpath, skip_nodata=False)
            self.assertTrue(os.path.isfile(outpath))
            self.assertEqual(_get_n_records(f'{outpath[:-4]}.dbf'), n_all_points)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(os.path.isfile(outpath))
            self.assertEqual(_get_n_records(f'{outpath[:-4]}.dbf'), n_all_points - n_outliers)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(os.path.isfile(outpath))
            self.assertEqual(_get_n_records(f'{outpath[:-4]}.dbf'), n_all_points - n_nodata - n_outliers)
